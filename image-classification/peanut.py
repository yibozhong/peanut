...
class peanut_vanilla(nn.Module):
    """
    vanilla implementation of PEANuT with no intermediate layers, no residual, depth = 2 (A and B)
...
class peanut_multilayers_depth_6(nn.Module):
    """
    Simpler naive implementation of PEANuT with multilayers.
    We actually use this version in the paper for the analysis, and it DOES seem to perform better.
    However, in order to easily stack up multiple layers flexibly, a more general implementation is provided (see peanut_multilayers below).
    """
    def __init__(self, dim=32, out_dim=768):
        super().__init__()
        self.non_linear = nn.ReLU()
...
class peanut_multilayers(nn.Module):
    """
    Implementation of PEANuT with residual and intermediate layers
    use ReLU as the default non-linear activation
    args:
        dim: hidden dimension (a.k.a. rank)
        out_dim: output dimension
        depth: number of adapters in PEANuT (A, B and all the intermediate layers)
    """
...
def set_peanut_vanilla(model, peanut_mode=1, mhsa_dim=16, ffn_dim=16):
    """
    peanut mode:
        1: vanilla peanut on mhsa q, v
        2: vanilla peanut on both mhsa q, v and ffn
    """
    for name, layer in model.named_children():
        if 'attention' in name:
            layer.attention.peanut_mode = peanut_mode
            layer.attention.q_adapter = peanut_vanilla(dim=mhsa_dim)
            layer.attention.v_adapter = peanut_vanilla(dim=mhsa_dim)
            bound_method = forward_attn.__get__(layer.attention, layer.attention.__class__)
            setattr(layer.attention, 'forward', bound_method)
        elif 'dense' in name:
            if peanut_mode == 1:
                continue
            layer.peanut_mode = peanut_mode
            layer.adapter = peanut_vanilla(dim=ffn_dim, out_dim=layer.weight.shape[0])
            bound_method = forward_ffn.__get__(layer, layer.__class__)
            setattr(layer, 'forward', bound_method)
        elif len(list(layer.children())) != 0:
            set_peanut_vanilla(layer, peanut_mode, mhsa_dim, ffn_dim)


def set_peanut_multilayers(model, peanut_mode=1, mhsa_dim=16, ffn_dim=16, depth=2):
    """
        peanut mode:
        1: peanut with more depth on mhsa q, v (qv setting)
        2: peanut with more depth on both mhsa q, v and ffn (qvmlp setting)
    """
    for name, layer in model.named_children():
        if 'attention' in name:
            layer.attention.peanut_mode = peanut_mode
            layer.attention.q_adapter = peanut_multilayers(dim=mhsa_dim, depth=depth)
            layer.attention.v_adapter = peanut_multilayers(dim=mhsa_dim, depth=depth)
            bound_method = forward_attn.__get__(layer.attention, layer.attention.__class__)
            setattr(layer.attention, 'forward', bound_method)
        elif 'dense' in name:
            if peanut_mode == 1:
                continue
            layer.peanut_mode = peanut_mode
            layer.adapter = peanut_multilayers(dim=ffn_dim, out_dim=layer.weight.shape[0])
            bound_method = forward_ffn.__get__(layer, layer.__class__)
            setattr(layer, 'forward', bound_method)
        elif len(list(layer.children())) != 0:
            set_peanut_multilayers(layer, peanut_mode, mhsa_dim, ffn_dim, depth=depth)
