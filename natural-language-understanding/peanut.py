...
class peanut_vanilla(nn.Module):
    """
    vanilla implementation of PEANuT, no residual, no depth
...
def set_peanut_s(model, s=1.0):
    """
    small version of PEANuT, only adapting layer 4 and onward, with rank = 1
    """
    for n, layer in model.named_modules():
        if n.endswith(('self')) and ('10' in n or '11' in n or '8' in n or '9' in n or '6' in n or '7' in n or '5' in n or '4' in n):
            layer.q_adapter = peanut_vanilla(dim=1)
            layer.v_adapter = peanut_vanilla(dim=1)
            bound_method = forward_attn.__get__(layer, layer.__class__)
            layer.s = s
            setattr(layer, 'forward', bound_method)

def set_peanut_l(model, dim=8, s=1.0):
    """
    large version of PEANuT, adapting all the layers
    default rank is 8 to but it can be flexibly set
    """
    for n, layer in model.named_modules():
        if n.endswith(('self')):
            layer.q_adapter = peanut_vanilla(dim=dim)
            layer.v_adapter = peanut_vanilla(dim=dim)
            bound_method = forward_attn.__get__(layer, layer.__class__)
            layer.s = s
            setattr(layer, 'forward', bound_method)